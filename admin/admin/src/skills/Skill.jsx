...
import { deleteSkill, skills, skillStatus } from "../ApiServices/ApiCallService";
...
const Skill = () => {
  ...
  const handleSkills = async () => {
    try {
      const res  = await skills(searchTerm, currentPage, 10);
      if (res && !res?.error) {
        setDetails(res?.data?.data?.skills || []);
  ...
  const handleStatus = async (id, isActive) => {
    try {
      const res = await skillStatus(id, {isActive});
      if (res?.data?.success) {
        handleSkills();
      }
    } catch (error) {
